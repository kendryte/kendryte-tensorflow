...
==============================================================================*/
#ifndef TENSORFLOW_LITE_KERNELS_INTERNAL_KPU_KPU_OPS_H_
#define TENSORFLOW_LITE_KERNELS_INTERNAL_KPU_KPU_OPS_H_
...
inline void Conv(const ConvParams& params, const RuntimeShape& input_shape,
  ...
  const uint32_t output_depth = MatchingDim(filter_shape, 0, output_shape, 3);
  if (bias_data)
  {
  ...
  // init kernels
  {
    ...
    for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
      for (int in_channel = 0; in_channel < input_depth; ++in_channel) {
        for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
          for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
            *k_it++ = filter_data[Offset(filter_shape, out_channel, filter_y,
                filter_x, in_channel)];
          }
        }
      }
...
#endif  // TENSORFLOW_LITE_KERNELS_INTERNAL_KPU_KPU_OPS_H_
